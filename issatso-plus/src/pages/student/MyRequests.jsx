...
function MyRequests({ onOpenDetails }) {
  const { user } = useAuth()
  ...
  useEffect(() => {
    const fetchRequests = async () => {
      try {
        ...
        setErrorMessage('')
        if (!user?.cin) {
          throw new Error("CIN utilisateur introuvable.")
        }
        const data = await getMyRequests(user.cin)
        setRequests(data)
      } catch (error) {
        setErrorMessage(error.message || 'Erreur lors du chargement des demandes.')
      } finally {
        setLoading(false)
      }
    }
  ...
  const filteredRequests = useMemo(() => {
    return requests.filter((item) => {
      const matchesSearch =
        ...
        item.nomEntreprise?.toLowerCase().includes(search.toLowerCase()) ||
        item.studentComment?.toLowerCase().includes(search.toLowerCase())
      const matchesStatus =
      ...
      <main className="px-8 pb-10 pt-8">
        <div className="mb-8 rounded-sm border border-[#e5e5e5] bg-white px-5 py-4">
          ...
          <p className="mt-2 text-[13px] text-[#6b7280]">
            Consultez l'état d'avancement et l'historique de vos demandes.
          </p>
          ...
          <div className="space-y-6">
            {filteredRequests.map((item) => {
              ...
              return (
                <article
                  key={item.id}
                  className="rounded-sm border border-[#e4e4e4] bg-white px-6 py-5"
                >
                  <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                    <div className="min-w-0">
                      ...
                      <p className="mt-3 text-[14px] leading-7 text-[#5f6470]">
                        {item.studentComment || 'Aucun commentaire étudiant.'}
                      </p>
                      <div className="mt-4 space-y-1 text-[13px] text-[#7b8794]">
                        <p>Entreprise : {item.nomEntreprise || '-'}</p>
                        <p>Filière : {item.filiere || '-'}</p>
                        <p>Date création : {item.createdAt?.replace('T', ' ').slice(0, 16) || '-'}</p>
                      </div>
                    </div>
